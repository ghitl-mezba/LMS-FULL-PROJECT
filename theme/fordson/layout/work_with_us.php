...
$record1 =  $DB->get_records_sql('SELECT * FROM  lms_about_org where  lms_about_org.type = 3');
$record2 =  $DB->get_records_sql('SELECT * FROM  lms_about_org_details where lms_about_org_details.type = 3 ');
$record3 =  $DB->get_records_sql('SELECT * FROM  lms_breadcrumbs ');

$templatecontext['show_work_with_us1'] = '';
$templatecontext['show_work_with_us2'] = '';
$templatecontext['show_heading'] = '';

foreach ($record3 as $record) {
  $templatecontext['show_heading'] .=  '    <section class="inner-header divider parallax" data-bg-img="' . theme_url() . '/settings/uploads/' . $record->image_work . '">
  <div class="container pt-70 pb-20">
    <div class="section-content">
      <div class="row">
        <div class="col-md-12">
          <h2 class="title text-white text-center">' . $record->heading_work . '</h2>
          <ol class="breadcrumb text-left mt-10">
            <li><a href="#" class="text-white">Home</a></li>
            <li><a href="#" class="text-white">Pages</a></li>
            <li class="active text-white">' . $record->heading_work . '</li>
          </ol>
        </div>
      </div>
    </div>
...
foreach ($record1 as $record) {
  if ($y == 1) {
    $templatecontext['show_work_with_us1'] .=  '<div class="section-title text-center">
      <div class="row">
...
foreach ($record2 as $record) {
  $templatecontext['show_work_with_us2'] .=  '<div class="item">
...
echo $OUTPUT->render_from_template('theme_fordson/fronttemplate/work_with_us', $templatecontext);
